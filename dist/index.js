...
var ReactImage = function (_React$Component) {
    _inherits(ReactImage, _React$Component);

    function ReactImage(props) {
        _classCallCheck(this, ReactImage);

        var _this = _possibleConstructorReturn(this, (ReactImage.__proto__ || Object.getPrototypeOf(ReactImage)).call(this, props));

        _this.onLoad = function () {
            if (_this.$image) {
                _this.setState({
                    isLoaded: true,
                    className: _this.refreshCls('loadedCls')
                }, function () {
                    _this.$log('ReactImage \u63D0\u793A: \u8D44\u6E90\u52A0\u8F7D\u6210\u529F ' + _this.props.src);
                });
            }
        };
        ...
        _this.onError = function () {
            if (_this.$image) {
                _this.setState({
                    isLoaded: false,
                    isLoading: false,
                    className: _this.refreshCls('errorCls')
                }, function () {
                    _this.$log('ReactImage 提示: 资源加载失败');
                });
            }
        };
        ...
        _this.init();
        _this.state = {
            className: _this.refreshCls(props.src ? 'loadingCls' : 'emptyCls'),
            isLoading: !!props.src,
            isLoaded: false
        };
        return _this;
    ...
    _createClass(ReactImage, [{
        key: 'init',
        value: function init() {
            this.status = ['loading', 'empty', 'error'];
            this.statusCls = ['loadingCls', 'emptyCls', 'errorCls', 'loadedCls'];
            this.$log = this.props.debug ? this.consoleLog('debug') : noop;
        }
    }, {
        ...
        value: function componentDidMount() {
            ...
            if (isLoading) {
                src ? this.$log('ReactImage 提示: 资源正在加载中') : this.$log('ReactImage 提示: 资源不存在');
        ...
        value: function componentWillReceiveProps(nextProps) {
            ...
            // 判断资源地址是否改变，若资源存在则加载之
            if (this.props.src !== nextProps.src) {
                this.setState({
                    isLoading: !!nextProps.src,
                    isLoaded: false
                }, function () {
                    nextProps.src ? _this2.$log('ReactImage 提示: 资源正在加载中') : _this2.$log('ReactImage 提示: 资源不存在');
                    nextProps.src && _this2.loadImage(nextProps.src);
                });
            }
        }
        ...
        key: 'consoleLog',
        value: function consoleLog(type) {
        ...
        key: 'getImageProps',
        value: function getImageProps() {
            var props = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

            var _props = Object.assign({}, props);
            var _items = ['debug'].concat(_toConsumableArray(this.status), _toConsumableArray(this.statusCls));
        ...
        key: 'refreshCls',
        value: function refreshCls(cls) {
            var _this3 = this;

            var className = (this.props.className || '').replace(/^\s*|\s*$/g, '').split(' ');
            var refreshCls = {};
            var _items = [].concat(_toConsumableArray(className), _toConsumableArray(this.statusCls));

            _items.forEach(function (n) {
                if (_this3.statusCls.includes(n)) {
                    var key = _this3.props[n];
                    refreshCls[key] = n === cls;
                } else {
                    refreshCls[n] = true;
                }
            });

            return (0, _classnames2.default)(refreshCls);
        }
         ...
         * 渲染其他组件
         */
        value: function renderView(props, src) {
            var imgProps = Object.assign({}, props, {
                src: src,
                onLoad: noop,
                onError: noop
            });

            return typeof src === 'string' ? _react2.default.createElement('img', imgProps) : src;
        }
    }, {
        key: 'render',
        value: function render() {
            ...
            var imgProps = Object.assign({}, this.getImageProps(this.props), {
                src: src,
                className: className
            });
            ...
            // 资源不存在
            if (!isLoading && !src) {
                return this.renderView(imgProps, empty);
            }

            // 资源正在加载中
            if (!isLoaded && isLoading) {
                return this.renderView(imgProps, loading);
            }

            // 资源加载失败
            if (!isLoaded && !isLoading) {
                return this.renderView(imgProps, error);
            }
...
ReactImage.propTypes = {
    ...
    loading: (0, _propTypes.oneOfType)([_propTypes.string, _propTypes.node]),
    loadingCls: _propTypes.string,
    loadedCls: _propTypes.string,
    error: (0, _propTypes.oneOfType)([_propTypes.string, _propTypes.node]),
    errorCls: _propTypes.string,
    empty: (0, _propTypes.oneOfType)([_propTypes.string, _propTypes.node]),
    emptyCls: _propTypes.string,
