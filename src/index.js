...
class ReactImage extends React.Component {
    constructor(props) {
        ...
        this.init()
        this.state = {
            className: this.refreshCls(props.src ? 'loadingCls' : 'emptyCls'),
            isLoading: !!props.src,
            isLoaded: false,
        }
    }
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (this.props.src !== nextProps.src) {
            this.setState({
                ...
                nextProps.src ? this.$log('ReactImage 提示: 资源正在加载中') : this.$log('ReactImage 提示: 资源不存在')
                nextProps.src && this.loadImage(nextProps.src)
            })
        }
    }
    ...
    consoleLog(type) {
        ...
        return (...args) => logFn.apply(console, args)
    }
    ...
    getImageProps(props = {}) {
        ...
        const _items = ['debug', ...this.status, ...this.statusCls]
     ...
     */
    refreshCls(cls) {
        const className = (this.props.className || '').replace(/^\s*|\s*$/g, '').split(' ')
        const refreshCls = {}
        const _items = [...className, ...this.statusCls]

        _items.forEach((n) => {
            if (this.statusCls.includes(n)) {
                const key = this.props[n]
    ...
    onLoad = () => {
        if (this.$image) {
            this.setState({
                isLoaded: true,
                className: this.refreshCls('loadedCls'),
            }, () => {
                this.$log(`ReactImage 提示: 资源加载成功 ${this.props.src}`)
            })
        }
    }
    ...
    onError = () => {
        if (this.$image) {
            this.setState({
                isLoaded: false,
                isLoading: false,
                className: this.refreshCls('errorCls'),
            }, () => {
                this.$log('ReactImage 提示: 资源加载失败')
            })
        }
    }
    ...
    renderView(props, src) {
        ...
        return typeof src === 'string' ? <img {...imgProps} /> : src
    }

    render() {
        const { isLoading, isLoaded, className } = this.state
        const { src, loading, error, empty } = this.props
        const imgProps = Object.assign({}, this.getImageProps(this.props), {
            src,
            className,
        })
